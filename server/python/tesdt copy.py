...
# @cross_origin(supports_credentials=True)
def update_graphs(rows,active_cell,store_data,derived_virtual_selected_rows,derived_virtual_data):
    ...
    # the component.
     ...
     #kolla bara på time pag erefreshed, uppdatera i slutet här
    ...
    #   normed_score = normed_grades['Betyg'][index]
        ...
        for ind in map_course_to_classes["Klass"]:
            if ind not in classes_df["Ämnesområde"]:
                index = list(averg_grade_df["Klass"]).index(ind)
                aver_grade = averg_grade_df["Averg. score"][index]
                classes_df["Ämnesområde"].append(ind)
                classes_df["Antal kurser"].append(1)
                classes_df["Medelbetyg"].append(aver_grade)
            else:
                index = list(classes_df["Ämnesområde"]).index(ind)
                updated_nbr = classes_df["Antal kurser"][index] + 1
                classes_df["Antal kurser"][index] = updated_nbr
        
        fig = px.pie(classes_df, values= "Antal kurser", names="Ämnesområde", title="hej",hover_data=["Medelbetyg"],hole =.3)
        return {"fig":fig, "classes_df":classes_df["Ämnesområde"]}
    ...
    df_grade_class_occurence_plot = pd.DataFrame({
            "Ämnesområden": unique_class_occurence_plot,#string
    ...
    fig_grade_occurence = px.bar(df_grade_class_occurence_plot, x="Ämnesområden", y="Antal kurser", color="Betyg", barmode="group")
